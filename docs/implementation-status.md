...
Updated September 25, 2026. The original local acceptance results are recorded below. The app is now deployed on Vercel and Railway, with manual Telegram launch, group setup, enrollment, event creation, and comment-notification checks. The remaining release checklist is in [deployment.md](deployment.md).

| Phase | Implementation | Executed local acceptance | Live acceptance |
| --- | --- | --- | --- |
| 0 Bootstrap | Complete | Next production build; PostgreSQL Testcontainers readiness; clean Flyway migrations through V10; Hibernate validation | Hosted CI verified |
| 1 Authentication | Complete | HMAC tampering, expiry, future/malformed data rejection; user upsert; `/me`; browser memory-only session and expiry | Signed launch checked manually |
| 2 Communities | Complete | Setup roles; transactional rollback/deduplication; nonmember denial; leaving/rejoining; same-second out-of-order updates | Group setup and enrollment checked manually; role changes and removal still pending |
...
| 6 Discussion | Complete | Section/parent restrictions; reply depth; soft deletion; two-client comment visibility; pagination UI | Comment posting checked manually; broader group testing pending |
...
| 9 Notifications | Complete | Retry-after; fenced lease recovery; mute filtering; blocked bot disables DM; reminder uniqueness; inactive-community history filtering | Comment notification delivery checked manually; blocked bot and reminder timing pending |
...
| 14 Launch | Deployed; release checks remain | Configuration examples and release checklist prepared | Hosting and HTTPS configured; backups, monitoring, and cohort checks pending |

## Baseline local checks (September 24)
...
Calendar imports, backup/restore drills, mobile-client coverage, and cohort usage measurement remain pending. The September 25 timezone and unified-discussion update passed backend verification, 10 frontend tests, and five Chromium workflows.
